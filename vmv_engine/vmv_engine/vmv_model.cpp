...
namespace vmv {
	vmvModel::vmvModel(vmvDevice& device, const std::vector<Vertex>& vertices) : vmv_device{ device } {
	...
	void vmvModel::createVertexBuffers(const std::vector<Vertex>& vertices) {
		...
		vmv_device.createBuffer(bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, vertexBuffer, vertexBufferMemory);
		void* data;
		vkMapMemory(vmv_device.device(), vertexBufferMemory, 0, bufferSize, 0, &data);
		memcpy(data, vertices.data(), static_cast<size_t>(bufferSize));
		vkUnmapMemory(vmv_device.device(), vertexBufferMemory);
	}
