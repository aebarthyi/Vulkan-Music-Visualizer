...
namespace ae {
    ...
    public:
        ...
        void update(std::vector<aeGameObject>& objs, float dt, unsigned int substeps = 1) {
        ...
        glm::vec2 computeForce(aeGameObject& fromObj, aeGameObject& toObj) const {
    ...
    private:
        void stepSimulation(std::vector<aeGameObject>& physicsObjs, float dt) {
    ...
    public:
        void update(
            ...
            std::vector<aeGameObject>& physicsObjs,
            std::vector<aeGameObject>& vectorField) {
    ...
    std::unique_ptr<aeModel> createSquareModel(aeDevice& device, glm::vec2 offset) {
    ...
    std::unique_ptr<aeModel> createCircleModel(aeDevice& device, unsigned int numSides) {
        std::vector<aeModel::Vertex> uniqueVertices{};
    ...
    void FirstApp::run() {
        // create some models
        std::shared_ptr<aeModel> squareModel = createSquareModel(
            ae_Device,
            { .5f, .0f });  // offset model by .5 so rotation occurs at edge rather than center of square
        std::shared_ptr<aeModel> circleModel = createCircleModel(ae_Device, 64);

        // create physics objects
        std::vector<aeGameObject> physicsObjects{};
        auto red = aeGameObject::createGameObject();
        ...
        auto blue = aeGameObject::createGameObject();
        ...
        for (int i = 0; i < gridCount; i++) {
            for (int j = 0; j < gridCount; j++) {
                auto vf = aeGameObject::createGameObject();
    ...
    void FirstApp::loadGameObjects() {
        ...
        auto ae_Model = std::make_shared<aeModel>(ae_Device, vertices);

        auto triangle = aeGameObject::createGameObject();
        triangle.model = ae_Model;
